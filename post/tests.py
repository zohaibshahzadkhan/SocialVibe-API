...
class ViewTests(TestCase):
    ...
    def test_post_list_profile(self):
        ...
        response = self.client.get(
            reverse("post_list_profile", kwargs={"id": self.user2.id})
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_post_like(self):
        ...
        response = self.client.post(
            reverse("post_like", kwargs={"pk": post_to_like.id})
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_post_create_comment(self):
        ...
        response = self.client.post(
            reverse("post_create_comment", kwargs={"pk": self.post1.id}),
            {
    ...
    def test_post_delete(self):
        ...
        response = self.client.delete(
            reverse("post_delete", kwargs={"pk": self.post1.id})
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Post.objects.count(), initial_count - 1)
